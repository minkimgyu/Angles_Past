...
public class Player : Avatar<Player.State>
{
        ...
        BuffInt dashCount, BuffFloat dashDuration, BuffFloat dashThrust, BuffFloat dashRecoverRatio, string[] skillNames)
    {
        ...
        m_readySpeedDecreaseRatio = readySpeedDecreaseRatio.CopyData();
        m_rushThrust = rushThrust.CopyData();
        m_rushRecoverRatio = rushRecoverRatio.CopyData();
        m_rushDuration = rushDuration.CopyData();

        m_attackCancelOffset = attackCancelOffset;

        m_dashCount = dashCount.CopyData();
        m_dashDuration = dashDuration.CopyData();
        m_dashThrust = dashThrust.CopyData();
        m_dashRecoverRatio = dashRecoverRatio.CopyData();
    }
    ...
    [SerializeField]
    public PlayerActionEventSO hpUIEventSO;
    ...
    public void AddInitialStat()
    {
        ...
        Dictionary<string, float> abilityPoints = SaveManager.Instance.ReturnAbilityUpgradePoint();

        Hp.IntervalValue *= abilityPoints["Hp"];
        RushDuration.IntervalValue *= abilityPoints["Shooting"];
        DashThrust.IntervalValue *= abilityPoints["Dash"];

        hpUIEventSO.OnActionRequested(Hp.IntervalValue);
    }

    public void AddAdditionalStat()
    {
        Weight.IntervalValue += 0.01f;
    }

    public override void UnderAttack(float damage, Vector2 dir, float thrust)
    {
        if (Immortality == true) return;
    ...
    public void SubtractDashRatio() => m_dashRatio -= (float)1 / m_dashCount.IntervalValue;
